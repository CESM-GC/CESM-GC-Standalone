...
!===============================================================================
  ...
  subroutine chem_timestep_tend( state, ptend, cam_in, cam_out, dT, pbuf, fh2o )
    ...
    qH2O(:nY,:nZ)   = REAL(state%q(:nY,:nZ,cQ),r8)
    h2ovmr(:nY,:nZ) = qH2O(:nY,:nZ) * MWDry / 18.016e+0_fp
    State_Chm(LCHNK)%Species(1,:nY,nZ:1:-1,iH2O) = REAL(qH2O(:nY,:nZ),fp)
 ...
 110            FORMAT( '--> ', a, ' will use prescribed surface boundary ',   &
    ...
    DO J = 1, nY
       ...
       ENDDO
       O3col(J) = O3col(J) * ( AVO / MWO3 ) / 2.687e+17_fp
    ENDDO
