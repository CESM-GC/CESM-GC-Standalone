...
def single_case(opts_dict, case_flags, stat_dir):
    ...
    # To run with HEMCO:
    command = './xmlchange USE_ESMF_LIB=TRUE'
    ret = os.system(command)
    command = './xmlchange ESMF_LOGFILE_KIND=ESMF_LOGKIND_SINGLE'
    ret = os.system(command)

    # Specify number of tasks
    command = 'TASKS=576; ./xmlchange NTASKS_CPL=$TASKS; ./xmlchange NTASKS_ATM=$TASKS; ./xmlchange NTASKS_LND=$TASKS; ./xmlchange NTASKS_ICE=$TASKS; ./xmlchange NTASKS_OCN=$TASKS ;./xmlchange NTASKS_ROF=$TASKS; ./xmlchange NTASKS_GLC=$TASKS; ./xmlchange NTASKS_WAV=$TASKS;'
    ret = os.system(command)
    command = 'THRDS=1; ./xmlchange NTHRDS_CPL=$THRDS; ./xmlchange NTHRDS_ATM=$THRDS; ./xmlchange NTHRDS_LND=$THRDS; ./xmlchange NTHRDS_ICE=$THRDS; ./xmlchange NTHRDS_OCN=$THRDS ;./xmlchange NTHRDS_ROF=$THRDS; ./xmlchange NTHRDS_GLC=$THRDS; ./xmlchange NTHRDS_WAV=$THRDS;'
    ret = os.system(command)

    # Specify lengths of run and start date
    command = './xmlchange RUN_STARTDATE="2015-01-01"'
    ret = os.system(command)
    command = './xmlchange --subgroup case.run JOB_WALLCLOCK_TIME="12:00:00";'
    ret = os.system(command)
    command = './xmlchange STOP_N=4; ./xmlchange STOP_OPTION=nmonths;'
    ret = os.system(command)
    command = './xmlchange REST_N=4; ./xmlchange REST_OPTION=nmonths;'
    ret = os.system(command)
    command = './xmlchange RESUBMIT=30;'
    ret = os.system(command)
